...
AUTH_HEADERS = {"Authorization": "Bearer test-token-for-pytest"}
...
def client():
    ...
    return TestClient(api_module.app, raise_server_exceptions=True, headers=AUTH_HEADERS)
...
def client_and_handle():
    ...
    api_module._temporal = temporal
    api_module._store = StubKnowledgeStore()
    return TestClient(api_module.app, raise_server_exceptions=True, headers=AUTH_HEADERS), handle
...
def test_health_temporal_ok():
    ...
    c = TestClient(api_module.app, headers=AUTH_HEADERS)
    r = c.get("/health")
