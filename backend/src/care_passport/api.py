...
def require_token(credentials: HTTPAuthorizationCredentials | None = Depends(_bearer)) -> None:
    ...
    if not hmac.compare_digest(token.encode(), settings.api_auth_token.encode()):
...
async def create_patient(body: CreatePatientBody, _: None = Depends(require_token)):
    _, status = await _start_workflow(body.id)
    return {"patient_id": body.id, "workflow_id": _wf_id(body.id), "status": status}


@app.get("/patients")
async def list_patients(_: None = Depends(require_token)):
...
async def delete_patient(patient_id: str, _: None = Depends(require_token)):
    handle = await _get_handle(patient_id)
...
async def vapi_end_of_call(body: VapiEndOfCallBody, patient_id: str = Query(...), _: None = Depends(require_token)):
    handle = await _get_handle(patient_id)
...
async def post_contribution(patient_id: str, body: ContributionBody, _: None = Depends(require_token)):
    handle = await _get_handle(patient_id)
    payload = {
        "source": "contribution",
        "text": body.text,
